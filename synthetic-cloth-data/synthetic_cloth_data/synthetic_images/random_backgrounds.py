...
def create_random_background_dataset(coco_json_path: str, target_dir: str, backgrounds_dir: str, num_backgrounds_per_sample:int = 1):
    ...
    backgrounds = [os.path.join(backgrounds_dir, f) for f in os.listdir(backgrounds_dir) if f.endswith(".jpg") or f.endswith(".png")]
    ...
    for annotation in tqdm.tqdm(coco_dataset.annotations):
        for _ in range(num_backgrounds_per_sample):
            ...
            segmentation_mask = BinarySegmentationMask.from_coco_segmentation_mask(annotation.segmentation,coco_image.width,coco_image.height)
            ...
            new_coco_images.append(CocoImage(id=new_image_id, file_name=f"images/{new_image_id}.jpg",width=coco_image.width, height=coco_image.height))
    ...
    dataset = CocoKeypointsDataset(images=new_coco_images, annotations=new_coco_annotations, categories=coco_dataset.categories)
    with open(target_dir / "annotations.json", "w") as f:
        f.write(dataset.json(indent=2))

if __name__ == "__main__":

    coco_json = "/home/tlips/Code/synthetic-cloth-data/synthetic-cloth-data/data/datasets/TOWEL/pyflex/annotations.json"
